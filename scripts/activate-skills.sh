...
if [[ ! -s "$SKILLS_LIST_FILE" ]]; then
  if [[ "${#EXTRA_ARGS[@]}" -eq 0 ]]; then
    printf '%s\n' \
      "frontend-development/verification-before-completion" \
      "frontend-development/writing-plans" \
      "frontend-development/first-ask" \
      "frontend-development/frontend-design" \
      "frontend-development/frontend-dev" \
      "frontend-development/shadcn-ui" \
      "marketing-and-seo/seo-agent" \
      "marketing-and-seo/copywriting" \
      "marketing-and-seo/marketing-ideas" >"$SKILLS_LIST_FILE"
  else
    : >"$SKILLS_LIST_FILE"
...
while IFS= read -r skill_id || [[ -n "$skill_id" ]]; do
  ...
  if [[ -e "$SKILLS_DIR/$skill_id" ]]; then
    ...
    echo "  + $skill_id"
    # Recreate the path inside the target dir if it contains subdirectories
    mkdir -p "$(dirname "$SKILLS_DIR/$skill_id")"
    cp -RP "$LIBRARY_DIR/$skill_id" "$SKILLS_DIR/$skill_id"
  else
