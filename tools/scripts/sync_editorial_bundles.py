...
REPO_URL = "https://github.com/aboalrejal-ai/skills"
AUTHOR = {
    "name": "aboalrejal-ai and contributors",
    "url": REPO_URL,
}
ROOT_CLAUDE_PLUGIN_NAME = "aboalrejal-skills"
ROOT_CODEX_PLUGIN_NAME = "aboalrejal-skills"
ROOT_CLAUDE_PLUGIN_DIRNAME = "aboalrejal-skills-claude"
...
def _bundle_plugin_name(bundle_id: str) -> str:
    return f"aboalrejal-bundle-{bundle_id}"


def _bundle_codex_plugin_name(bundle_id: str) -> str:
    return f"abob-{bundle_id}"
...
def _bundle_codex_long_description(bundle: dict[str, Any]) -> str:
    ...
    if not highlights:
        return f'{audience} Includes {len(bundle["skills"])} curated skills from Abo Alrejal Skills.'
...
def render_bundles_doc(
    ...
    template_path = root / EDITORIAL_TEMPLATE_PATH
    if template_path.is_file():
        template = template_path.read_text(encoding="utf-8")
    else:
        template = "# 🎒 Skill Bundles\n\n{{bundle_sections}}\n"
...
def _root_claude_plugin_manifest(metadata: dict[str, Any], supported_skill_count: int) -> dict[str, Any]:
    ...
    return {
        ...
        "description": (
            f"Plugin-safe Claude Code distribution of Abo Alrejal Skills with "
...
def _root_codex_plugin_manifest(metadata: dict[str, Any], supported_skill_count: int) -> dict[str, Any]:
    ...
    return {
        ...
        "version": metadata["version"],
        "description": "Plugin-safe Codex plugin for the Abo Alrejal Skills library.",
        "author": AUTHOR,
        "homepage": REPO_URL,
        "repository": REPO_URL,
        "license": "MIT",
        ...
        "skills": "./skills/",
        "interface": {
            "displayName": "Abo Alrejal Skills",
            "shortDescription": (
                f"{supported_label} plugin-safe skills for coding, security, product, and ops workflows."
            ),
            "longDescription": (
                "Install a plugin-safe Codex distribution of Abo Alrejal Skills. "
            ...
            "websiteURL": REPO_URL,
            "defaultPrompt": [
                "Use @frontend-development/writing-plans to plan a new feature.",
                "Use @frontend-development/verification-before-completion to verify a task.",
            ],
            "brandColor": "#0F172A",
        },
    }
...
def _bundle_claude_plugin_manifest(metadata: dict[str, Any], bundle: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "version": metadata["version"],
        "description": (
            f'Editorial "{bundle["name"]}" bundle for Claude Code from Abo Alrejal Skills.'
        ),
        "author": AUTHOR,
        "homepage": REPO_URL,
        "repository": REPO_URL,
...
def _bundle_codex_plugin_manifest(metadata: dict[str, Any], bundle: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "version": metadata["version"],
        "description": (
            f'Install the "{bundle["name"]}" editorial skill bundle from Abo Alrejal Skills.'
        ),
        "author": AUTHOR,
        "homepage": REPO_URL,
        "repository": REPO_URL,
...
def _render_claude_marketplace(
    ...
    plugins = [
        {
            ...
            "version": metadata["version"],
            "description": (
                "Expose the plugin-safe Claude Code subset of Abo Alrejal Skills "
    ...
    return {
        ...
        "metadata": {
            "description": (
                "Claude Code marketplace entries for the plugin-safe Abo Alrejal Skills "
...
def _replace_directory_atomically(
    ...
    def _safe_replace(src: Path, dst: Path, retries: int = 10, delay: float = 0.1) -> None:
        for attempt in range(retries):
            try:
                os.replace(src, dst)
                return
    ...
    try:
        ...
        if destination_root.exists() or destination_root.is_symlink():
            _safe_replace(destination_root, backup_root)
            replaced_existing = True

        _safe_replace(staging_root, destination_root)
    except Exception:
        if replaced_existing and backup_root.exists() and not destination_root.exists():
            try:
                _safe_replace(backup_root, destination_root)
            except Exception:
                pass
        raise
    finally:
...
def sync_editorial_bundle_plugins(
    ...
    for candidate in plugins_root.glob("aboalrejal-bundle-*"):
...
def main() -> int:
    ...
    if args.check:
        ...
        if current_doc != expected_doc:
            raise SystemExit("docs/users/bundles.md is out of sync")
        print("✅ Editorial bundles documents are in sync.")
        return 0

    sync_editorial_bundles(root)
    print("✅ Editorial bundles and plugin directories synchronized.")
    return 0
