...
- **Sole writer**: `memory-management` owns all wiki writes semantically. For [performance](../skills/backend-and-fullstack/performance), narrowly-scoped `memory/wiki/index.md` and `memory/wiki/<project>/index.md` auto-refreshes are delegated to the PostToolUse hook. Wiki log updates and compiled pages remain explicit `memory-management` operations using the schema documented in [memory-management SKILL.md §Wiki Layer](../skills/memory-management/SKILL.md). Any wiki-compiled page (entity/keyword/topic) still requires explicit `memory-management` invocation.
...
> Design archive: [proposal-wiki-layer-v3.md](./proposal-wiki-layer-v3.md). Active rules in this file and `memory-management` take precedence.

### COLD — `memory/archive/`

- No [capacity](../skills/cloud-and-infrastructure/capacity) limit
